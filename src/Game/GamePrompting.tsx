...
export default function GamePrompting() {
  ...
  if (
    roomstate.userGameState.topicPlace >= roomstate.userGameState.topics.length
  ) {
    return <Waiting />;
  }
  ...
  const doesPromptContainWordsFromTopic = (prompt: string): boolean => {
    ...
    let topic =
      roomstate.userGameState.topics[roomstate.userGameState.topicPlace].slice(
        0
      );
  ...
  const checkPromptAndSubmit = (prompt: string) => {
    ...
    if (doesPromptContainWordsFromTopic(trimmedPrompt)) {
      setPromptError("no using words from topic (the, of, a etc. are allowed)");
      return;
    }
    ...
    <Box sx={{ mx: "auto", maxWidth: 350 }}>
      <Box sx={{ display: "flex", flexDirection: "column" }}>
        ...
        <Paper sx={{ width: "fit-content", p: 1, my: 1 }}>
          {roomstate.userGameState.topics[roomstate.userGameState.topicPlace]}
        </Paper>
        <TextField
          value={prompt}
          error={promptError != ""}
